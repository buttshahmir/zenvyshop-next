        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setShowFilters(!showFilters)}
              className={`px-4 py-3 border rounded-lg flex items-center gap-2 transition-colors ${showFilters ? 'bg-gold text-black border-gold' : 'bg-white text-black border-gray-200'}`}>
          ...
          {CATEGORIES.map((cat) => (
            <button key={cat} onClick={() => handleCategoryChange(cat)}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                selectedCategory === cat ? 'bg-gold text-black' : 'bg-white text-gray-600 border border-gray-200 hover:border-gold hover:text-gold'}`}>
              {cat}
            </button>
          ))}
        </div>
        ...
        {showFilters && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setMinPrice(''); setMaxPrice(''); }} className="text-sm text-gold hover:text-gold-dark">Reset</button>
            </div>
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <input type="number" placeholder="Max" value={maxPrice} onChange={(e) => setMaxPrice(e.target.value)}
                className="w-32 px-3 py-2 border border-gray-200 rounded-lg" />
              <button onClick={fetchProducts} className="bg-gold text-black px-4 py-2 rounded-lg text-sm font-medium hover:bg-gold-light transition-colors">Apply</button>
            </div>
          </motion.div>
        )}

        {!loading && !error && (
          <p className="text-gray-500 text-sm mb-6">Showing {products.length} {products.length === 1 ? 'product' : 'products'}</p>
        )}

        {error && (
          <div className="text-center py-20">
            <p className="text-red-500 mb-4">{error}</p>
            <button onClick={fetchProducts} className="text-gold hover:text-gold-dark font-medium">Try again</button>
          </div>
        )}
        ...
        {!error && (
          loading ? (
            ...
            <div className="text-center py-20">
              ...
              <button onClick={clearFilters} className="mt-4 text-gold hover:text-gold-dark font-medium">Clear all filters</button>
            </div>
          )
        )}
      </div>
